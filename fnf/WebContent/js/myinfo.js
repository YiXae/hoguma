	...
	if(nick == "") {
		...
		$.ajax({				
			...
			success: function(data) {
				if(data.message == "-1"){
					mnick.parent().parent().next().css("display", "block").text("이미 사용중인 별명입니다.");
				} else if(data.message == "1"){
					mnick.parent().parent().next().css("display", "block").css("color", "#0000ff").text("멋진 별명이네요!");
				} 
			},			
			error: function(){
				alert("System Error!!!");
		...
		$.ajax({				
			...
			success: function(data) {
				if(data.message == "-1"){
					...
					mnick.parent().parent().next().css("display", "none");
					mpw.focus();
				} 
			},			
			error: function(){
