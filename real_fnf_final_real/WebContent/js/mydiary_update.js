...
$(".formstyle_write2").blur(function() {
	...
	if(inputval == "") {
		$(this).parent().parent().parent().parent().next().css("display", "block").text("111필수 정보입니다.");
	} else {
...
/* 게시글 종류 설정 시작 */
$("#board_kind").focus(function() {
	$(this).css("border", "1px solid orange");
});

$("#board_kind").blur(function() {
	$(this).css("border", "1px solid #dadada");
...
/* 농수산물 품목 설정 시작 */
$("#item_sel_again").blur(function() {
	$(this).css("border", "1px solid #dadada");
	
	var inputval = $(this).val();
	
	if(inputval == "품목 선택") {
		$("#itemagain").css("display", "block").text("필수 정보입니다.");
	} else {
		$("#itemagain").css("display", "none");
	}		
});	
/* 농수산물 대분류 설정 끝 */
...
$(document).on("click", "#ilji_update_btn", function() {	
	var
	form = $("#frm_boardUpdate"),
	title = $("#write_title_p"),
	open = $("#board_open"),
	item_again = $("#item_sel_again"),
	kind = $("#board_kind"),
	item = $("#item_sel"),
	...
	if(bkind == "게시글 종류") {
		...
		var bitem1 = $.trim(item.val());
		var bitem2 = $.trim(mainproduct.val());
		var bitem3 = $.trim(product.val());			
...
$(document).on("change","#uploadfile",function(){
	...
	if(filesize.length < 1){
		...
		if(size>maxSize){ //용량제한걸림
			alert("첨부파일 사이즈는 5mb이내로 등록 가능합니다.");
...
$(document).on("click","#ilji_update_btn",function(elClickedObj){
	...
	if(title==""){
		...
		return false;
	} $("#ilji_update_btn").submit();
	
});
/* 일지수정 버튼 클릭 시 스마트 에디터 값 저장 끝 */
