				...
				<td id="bodyalltd1">				
					...
					<div id="zone_box">									
						...
						<div class="my_detail_zone">							
							<form id="frm_boardUpdate" name="frm_boardUpdate" action="boardUpadteplay.bizpoll" method="POST" enctype="multipart/form-data">
									...
									<table class="write_ilji_table">
										...
										<tr>
											...
											<td class="write_ilji_table_td2">: 
												...
												<select id="item_sel_again" name="item_sel_again" class="formstyle_write4">
													<option value="품목 선택">품목 선택</option>
													<option value="농산물" <c:if test="${boardview.bitem.equals('농산물')}">selected</c:if>>농산물</option>
													<option value="수산물" <c:if test="${boardview.bitem.equals('수산물')}">selected</c:if>>수산물</option>
												</select>
											</td>
										</tr>
										<tr>
											<td colspan="2">
												<div class="writer_error" id="itemagain">
													필수정보 입니다.															
												</div>
											</td>											
										</tr>											
										<tr>
											...
											<td class="write_ilji_table_td2">: 
												<input type="hidden" id="bkindval" value="${boardview.bkind}">											
												...
												<td class="write_ilji_table_td2">																											
													...
													<div class="write_td_div3">														
														<select id="item_sel" name="item_sel" class="formstyle_write4">
														  ...
														  <option value="농산물" <c:if test="${boardview.bitema.equals('농산물')}">selected</c:if>>농산물</option>
														  <option value="수산물" <c:if test="${boardview.bitema.equals('수산물')}">selected</c:if>>수산물</option>
														</select>
														...
														<select id="mainproduct" name="mainproduct" class="formstyle_write4">
														  ...
														  <option class="농산물" value="곡물" <c:if test="${boardview.bmainproduct.equals('곡물')}">selected</c:if>>곡물</option>
														  <option class="농산물" value="야채" <c:if test="${boardview.bmainproduct.equals('야채')}">selected</c:if>>야채</option>
														  <option class="농산물" value="과일" <c:if test="${boardview.bmainproduct.equals('과일')}">selected</c:if>>과일</option>
														  <option class="수산물" value="어류" <c:if test="${boardview.bmainproduct.equals('어류')}">selected</c:if>>어류</option>
														  <option class="수산물" value="연체동물" <c:if test="${boardview.bmainproduct.equals('연제동물')}">selected</c:if>>연체동물</option>
														  <option class="수산물" value="해조류" <c:if test="${boardview.bmainproduct.equals('해조류')}">selected</c:if>>해조류</option>
														</select>
														...
														<select id="product" name="product" class="formstyle_write4">
														  ...
														  <option class="곡물" value="쌀" <c:if test="${boardview.bproduct.equals('쌀')}">selected</c:if>>쌀</option>
														  <option class="곡물" value="보리" <c:if test="${boardview.bproduct.equals('보리')}">selected</c:if>>보리</option>
														  <option class="곡물" value="현미" <c:if test="${boardview.bproduct.equals('현미')}">selected</c:if>>현미</option>
														  <option class="야채" value="배추" <c:if test="${boardview.bproduct.equals('배추')}">selected</c:if>>배추</option>
														  <option class="야채" value="상추" <c:if test="${boardview.bproduct.equals('상추')}">selected</c:if>>상추</option>
														  <option class="야채" value="시금치" <c:if test="${boardview.bproduct.equals('시금치')}">selected</c:if>>시금치</option>
														  <option class="과일" value="사과" <c:if test="${boardview.bproduct.equals('사과')}">selected</c:if>>사과</option>
														  <option class="과일" value="수박" <c:if test="${boardview.bproduct.equals('수박')}">selected</c:if>>수박</option>
														  <option class="과일" value="딸기" <c:if test="${boardview.bproduct.equals('딸기')}">selected</c:if>>딸기</option>
														  <option class="어류" value="고등어" <c:if test="${boardview.bproduct.equals('고등어')}">selected</c:if>>고등어</option>
														  <option class="어류" value="참치" <c:if test="${boardview.bproduct.equals('참치')}">selected</c:if>>참치</option>
														  <option class="어류" value="갈치" <c:if test="${boardview.bproduct.equals('갈치')}">selected</c:if>>갈치</option>
														  <option class="연체동물" value="오징어" <c:if test="${boardview.bproduct.equals('오징어')}">selected</c:if>>오징어</option>
														  <option class="연체동물" value="문어" <c:if test="${boardview.bproduct.equals('문어')}">selected</c:if>>문어</option>
														  <option class="연체동물" value="낙지" <c:if test="${boardview.bproduct.equals('낙지')}">selected</c:if>>낙지</option>
														  <option class="해조류" value="김" <c:if test="${boardview.bproduct.equals('김')}">selected</c:if>>김</option>
														  <option class="해조류" value="미역" <c:if test="${boardview.bproduct.equals('미역')}">selected</c:if>>미역</option>
														  <option class="해조류" value="파래" <c:if test="${boardview.bproduct.equals('파래')}">selected</c:if>>파래</option>														  
														</select>															 
													</div>
