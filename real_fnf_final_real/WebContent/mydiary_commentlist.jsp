...
<body>
	...
	<c:if test="${replyList.size() != 0}">
		...
		<fmt:formatDate value="${rtoday}" pattern="yyyy-MM-dd" var="today3"/>
    	<fmt:formatDate value="${replyview.regdate}" pattern="yyyy-MM-dd" var="regdate3"/>
			<div class="comment_rep" id="co3">
				<div class="comment_text_area">
					<div class="comment_writer">
						<span style="font-family: 'Nanum Gothic Coding', monospace;">
						<c:if test="${today3==regdate3}">
							<img src="img/board_new_rp.png" alt="" style="vertical-align: middle;">
						</c:if>
						<c:choose>
					    	<c:when test="${sessionScope.loginUser.mnick == replyview.writer}">
					    		<a href="javascript:;" class="js-open_my open-button_my">
								<b><span class="acolor">${replyview.writer}</span></b></a>&nbsp;
					    	</c:when>
					    	<c:otherwise>
					    		<a href="javascript:;" class="js-open_my open-button_my">
								<span class="acolor_any">${replyview.writer}</span></a>&nbsp;
					    	</c:otherwise>					    	
				    	</c:choose>						
						</span>
						<c:choose>
					    	<c:when test="${today3 == regdate3}">
